...
class Binning_CDF:
    ...
    print("aaaaa")
    @staticmethod
    def cut_log_generator(ncut, minimum, maximum):
        print("高斯")
        print("最小值是", minimum)
        print("最大值是", maximum)
        if minimum < 0:
            minimum = 1
        cut_points = np.linspace(np.log(minimum), np.log(maximum), num=ncut)
        cut_points = np.sort(cut_points)
        return cut_points
        ...
        if self.histogram_bin == 'random':
            ...
            for i in range(self.niter):
                ...
                if self.loss_model == 'multi-class':
                    #np.digitize返回输入数组中每个值所属的bin的索引。
            ...
            if self.loss_model == 'multi-binary-gaussian':
                fixed_cut = self.cut_log_generator(ncut, self.ylim[0], self.ylim[1])
            else:
            ...
            #np.digitize(b, a)返回b在a中的位置，a是一维数组，且单调
            if self.loss_model == 'multi-class':
                Train_label = np.digitize(train_y, fixed_cut)
                Valid_label = np.digitize(valid_y, fixed_cut)
            elif self.loss_model == 'multi-binary-gaussian':
                alpha = 15
                weight = [np.exp(-alpha*np.power(fixed_cut-x,2)) for x in fixed_cut]
                weight = np.asarray(weight,dtype=np.float32)
                Train_label = np.tile(fixed_cut, train_y.shape[0]).reshape(train_y.shape[0], -1)
                Train_label = (Train_label > train_y).astype(np.float32)
                #Train_label = tf.matmul(Train_label, weight)
                Train_label = np.dot(Train_label, weight).astype(np.int8)

                #Train_label = tf.cast(Train_label, dtype=np.int8)

                Valid_label = np.tile(fixed_cut, valid_y.shape[0]).reshape(valid_y.shape[0], -1)
                Valid_label = (Valid_label > valid_y).astype(np.float32)
                Valid_label = np.dot(Valid_label, weight).astype(np.int8)
            ...
            elif self.loss_model == 'multi-binary' or self.loss_model == 'multi-binary-gaussian':
            ...
            for i in range(self.niter):
                ...
                for j, nbin in enumerate(bin_ids):
                    ...
                    if y_grid[j] < self.ylim[0]:
                        ...
                        cdf_v = 1
                    elif self.loss_model == 'multi-binary' or self.loss_model == 'multi-crps' or \
                         self.loss_model == 'multi-binary-gaussian':
                        if nbin == 0:
